...
def train_ridge(hour):
    ...
    rdg.fit(hour_d_train_x, hour_d_train_y)
    return rdg
...
def train_and_persist(model_dir=None, hour_path=None, model = 'xgboost'):
    ...
    if model == "xgboost":
        trained_model = train_xgboost(hour)
    elif model == "ridge":
        trained_model = train_ridge(hour)

    model_path = get_model_path(model_dir=model_dir, model_name = model)
...
def predict(parameters, model_dir=None, model="xgboost"):
    ...
    result = final_model.predict(X_input)
